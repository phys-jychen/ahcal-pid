...
Int_t Select::ValidHits(const string& file, const string& tree)
{
    ...
    .Define("Hit_Time_nonzero", [] (vector<Double_t> Hit_Time, vector<Double_t> Digi_Hit_Energy, Int_t nhits)
    {
        vector<Double_t> Hit_Time_nonzero;
        for (Int_t i = 0; i < nhits; i++)
            if (Digi_Hit_Energy.at(i) > 0)
                Hit_Time_nonzero.emplace_back(Hit_Time.at(i));
        return Hit_Time_nonzero;
    }, {"Hit_Time", "Digi_Hit_Energy", "nhits"})
    .Define("Hit_X_nonzero", [] (vector<Double_t> Hit_X, vector<Double_t> Digi_Hit_Energy, Int_t nhits)
    {
    ...
    }, {"Hit_X", "Digi_Hit_Energy", "nhits"})
    .Define("Hit_Y_nonzero", [] (vector<Double_t> Hit_Y, vector<Double_t> Digi_Hit_Energy, Int_t nhits)
    {
    ...
    }, {"Hit_Y", "Digi_Hit_Energy", "nhits"})
    .Define("Hit_Z_nonzero", [] (vector<Double_t> Hit_Z, vector<Double_t> Digi_Hit_Energy, Int_t nhits)
    {
    ...
    .Define("Digi_Hit_Energy_nonzero", [] (vector<Double_t> Digi_Hit_Energy, Int_t nhits)
    {
        vector<Double_t> Digi_Hit_Energy_nonzero;
        for (Int_t i = 0; i < nhits; i++)
            if (Digi_Hit_Energy.at(i) > 0)
                Digi_Hit_Energy_nonzero.emplace_back(Digi_Hit_Energy.at(i));
        return Digi_Hit_Energy_nonzero;
    }, {"Digi_Hit_Energy", "nhits"})
    ...
    t->SetBranchStatus("*", 1);
    vector<TString> deactivate = { "CellID", "DetectorID", "Digi_Energy_ECAL", "Digi_Energy_HCAL", "Digi_Hit_Energy", "EventNum", "Hit_Time", "Hit_X", "Hit_Y", "Hit_Z", "nhits" };
