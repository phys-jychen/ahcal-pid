...
//        Int_t max_layer = maxPosition - layer_rms.begin();
    ...
    */
    ...
    .Define("COG_X_mean", [] (vector<Double_t> Hit_X_nonzero, vector<Double_t> Digi_Hit_Energy_nonzero, Double_t Edep, Int_t nhits)
    {
        ...
        {
            for (Int_t i = 0; i < nhits; i++)
                cog_x_mean += Hit_X_nonzero.at(i) * Digi_Hit_Energy_nonzero.at(i);
            cog_x_mean /= Edep;
            return cog_x_mean;
        }
    }, {"Hit_X_nonzero", "Digi_Hit_Energy_nonzero", "Edep", "nhits"})
    // The overall centre of gravity, in y direction
    .Define("COG_Y_mean", [] (vector<Double_t> Hit_Y_nonzero, vector<Double_t> Digi_Hit_Energy_nonzero, Double_t Edep, Int_t nhits)
    {
        ...
        {
            for (Int_t i = 0; i < nhits; i++)
                cog_y_mean += Hit_Y_nonzero.at(i) * Digi_Hit_Energy_nonzero.at(i);
            cog_y_mean /= Edep;
            return cog_y_mean;
        }
    }, {"Hit_Y_nonzero", "Digi_Hit_Energy_nonzero", "Edep", "nhits"})
    // The overall centre of gravity, in z direction
    .Define("COG_Z_mean", [] (vector<Double_t> Hit_Z_nonzero, vector<Double_t> Digi_Hit_Energy_nonzero, Double_t Edep, Int_t nhits)
    {
        ...
        {
            for (Int_t i = 0; i < nhits; i++)
                cog_z_mean += Hit_Z_nonzero.at(i) * Digi_Hit_Energy_nonzero.at(i);
            cog_z_mean /= Edep;
            return cog_z_mean;
        }
...
//    .Range(1)
    ...
    t->SetBranchStatus("*", 1);
    vector<TString> deactivate = { "COG_X_5", "COG_Y_5", "COG_Z_5", "Ecell", "Ecell_max_id", "FD_2D", "FD_3D", "Digi_Hit_Energy_nonzero", "Hit_Phi_nonzero", "Hit_Theta_nonzero", "Hit_Time_nonzero", "Hit_X_nonzero", "Hit_Y_nonzero", "Hit_Z_nonzero", "hits_on_layer", "layer", "layer_energy", "layer_rms", "layer_xwidth", "layer_ywidth" };
